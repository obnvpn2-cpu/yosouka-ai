...
**エンドポイント**:
1. `GET /` - ヘルスチェック
2. `GET /api/stats` - 全体統計取得
3. `GET /api/options` - 検索条件の選択肢取得
4. `POST /api/search` - 予想家検索
...
**使い方**:
1. APIサーバーを起動: `python backend/api/api.py`
2. ブラウザでフロントエンドを開く: `frontend/index.html`
3. 条件を選択して検索
4. おすすめの予想家が表示される
...
# 2. 予想履歴を取得
python backend/scraper/prediction.py

# 3. race_idを更新
python scripts/batch/batch_update_race_ids_v2.py

# 4. レース詳細を取得
python scripts/batch/batch_race_detail.py

# 5. 基本統計を再計算
python backend/analysis/calculate_basic_stats.py
```
...
1. **データベース確認**: `python scripts/check/check_phase4_data.py`
2. **API動作確認**: `python scripts/test/test_api.py`
3. **ログ確認**: `logs/` ディレクトリ
...
**現在地**: Phase 4完了、プロジェクト完成 🎉  
**次のマイルストーン**: 拡張機能の実装（オプション）
